...
static void set_profile_status_symbol(lv_obj_t *icon,
                                      struct profile_status_state state) {
  // char text[9] = {};
...
static void profile_status_update_cb(struct profile_status_state state) {
  ...
  SYS_SLIST_FOR_EACH_CONTAINER(&widgets, widget, node) {
    set_profile_status_symbol(widget->obj, state);
  }
}
...
int custom_widget_profile_status_init(
    struct custom_widget_profile_status *widget, lv_obj_t *parent) {
...
lv_obj_t *
custom_widget_profile_status_obj(struct custom_widget_profile_status *widget) {
  return widget->obj;
}
